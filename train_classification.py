# train.py
"""Train ConvKHRONOS (Encoder + KHRONOS head) on MNIST with train/val/test splits."""
...
from ckhronos import ConvKHRONOS, count_parameters


class TrainState(train_state.TrainState):
    """null."""

def load_mnist_splits() -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    labels_test = test_data["label"].astype(np.int32)
...
def train_step(state: TrainState, batch_images, batch_labels):
    ...
    def loss_fn(params):
        logits = state.apply_fn({"params": params}, batch_images.astype(jnp.float32))
        loss = optax.softmax_cross_entropy_with_integer_labels(
            logits=logits.astype(jnp.float32), labels=batch_labels
        ).mean()
        return loss, logits
...
def eval_step(state: TrainState, images, labels):
    logits = state.apply_fn({"params": state.params}, images.astype(jnp.float32))
    loss = optax.softmax_cross_entropy_with_integer_labels(
...
def main():
    ...
    labels_train = jnp.asarray(labels_train_np, dtype=jnp.int32)
    images_val = jnp.asarray(images_val_np, dtype=jnp.float32)
    labels_val = jnp.asarray(labels_val_np, dtype=jnp.int32)
    images_test = jnp.asarray(images_test_np, dtype=jnp.float32)
    labels_test = jnp.asarray(labels_test_np, dtype=jnp.int32)
    print(f"Train: {images_train.shape[0]}, Val: {images_val.shape[0]}, Test: {images_test.shape[0]} | image shape {images_train.shape[1:]}")
    ...
    print(f"Initialized model with {count_parameters(state.params):,} parameters.")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch_images, batch_labels in make_batches(images_train, labels_train, batch_size, batch_rng):
        ...
        eval_loss, eval_acc = eval_step(state, images_val, labels_val)
        epoch_time = time.time() - epoch_start
        print(
            f"Epoch {epoch:02d} | {epoch_time:4.1f}s | "
            f"train loss {avg_loss:.4f}, train acc {avg_acc*100:5.2f}% | "
            f"val loss {float(eval_loss):.4f}, val acc {float(eval_acc)*100:5.2f}%"
        )

    test_loss, test_acc = eval_step(state, images_test, labels_test)
    print(f"Training complete. Test loss {float(test_loss):.4f}, test acc {float(test_acc)*100:5.2f}%")
